...
from utils.yolo3_loss import YOLOV3Loss
...
__all__ = ['YOLOV3',
           ...
           'yolo3_mobilenet_v2_custom',
           'yolo3_shufflenet_v2_custom'
           ]
    ...
    def __init__(self, channel, norm_layer=BatchNorm, norm_kwargs=None, **kwargs):
        ...
        with self.name_scope():
            ...
            for _ in range(2):
                ...
                # 3x3 lds
                self.body.add(_conv2d_lds(channel * 2, 3, 1, 1,
                                          norm_layer=norm_layer, norm_kwargs=norm_kwargs))
...
def get_yolov3(name, stages, filters, anchors, strides, classes,
    ...
    '''
    assert not pretrained, 'no pre-train provided'
    return net
...
def yolo3_darknet53_custom(classes, transfer=None, pretrained_base=True, pretrained=False,
    ...
    if transfer is None:
        ...
        from utils.model_zoo import get_model
...
def yolo3_mobilenet1_0_custom(classes, transfer=None, pretrained_base=False, pretrained=False,
                              norm_layer=BatchNorm, norm_kwargs=None, **kwargs):
...
def yolo3_mobilenet_v2_custom(classes, transfer=None, pretrained_base=False, pretrained=False,
                              norm_layer=BatchNorm, norm_kwargs=None, **kwargs):
    """YOLO3 multi-scale with mobilenet base network on custom dataset.
    Parameters
    ----------
    ...
    if pretrained:
        warnings.warn("Custom models don't provide `pretrained` weights, ignored.")
    if transfer is None:
        base_net = get_mobilenet_v2(multiplier=1,
                                    pretrained=pretrained_base,
                                    norm_layer=norm_layer, norm_kwargs=norm_kwargs,
                                    **kwargs)
        stages = [base_net.features[:9],
                  base_net.features[9:16],
                  base_net.features[16:-1]]
        anchors = [
            [10, 13, 16, 30, 33, 23],
            [30, 61, 62, 45, 59, 119],
            [116, 90, 156, 198, 373, 326]]
        strides = [8, 16, 32]
        net = get_yolov3(
            'mobilenet_v2', stages, [512, 256, 128], anchors, strides, classes, '',
            norm_layer=norm_layer, norm_kwargs=norm_kwargs, **kwargs)
    else:
        from utils.model_zoo import get_model
        net = get_model(
...
def yolo3_shufflenet_v2_custom(classes, transfer=None, pretrained_base=True, pretrained=False,
                              norm_layer=BatchNorm, norm_kwargs=None, **kwargs):
    """YOLO3 multi-scale with ShuffleNet_v2 base network on custom dataset.
    Parameters
    ----------
    classes : iterable of str
        Names of custom foreground classes. `len(classes)` is the number of foreground classes.
    ...
    if pretrained:
        warnings.warn("Custom models don't provide `pretrained` weights, ignored.")
    if transfer is None:
        base_net = get_shufflenet_v2(multiplier=1.0)
        stages = [base_net.features[:8],
                  base_net.features[8:16],
                  base_net.features[16:-1]]
        anchors = [
            [10, 13, 16, 30, 33, 23],
            [30, 61, 62, 45, 59, 119],
        ...
        strides = [8, 16, 32]
        net = get_yolov3(
            'shufflenet_v2', stages, [512, 256, 128], anchors, strides, classes, '',
            norm_layer=norm_layer, norm_kwargs=norm_kwargs, **kwargs)
    else:
        from utils.model_zoo import get_model
        net = get_model(
            'yolo3_shufflenet_v2_' +
            str(transfer),
            pretrained=True,
            **kwargs)
        reuse_classes = [x for x in classes if x in net.classes]
