...
class InvertedResidual(nn.HybridBlock):
    def __init__(self, in_channels, out_channels, stride, stage=0):
        super().__init__()
        self.stride = stride
        ...
        with self.features.name_scope():
            ...
            for idx in range(len(self.stage_repeats)):
                ...
                for i in range(num_repeat):
                    if i == 0:
                        self.features.add(InvertedResidual(in_channel, out_channel, 2, stage=idx))
                    else:
                        self.features.add(InvertedResidual(in_channel, out_channel, 1, stage=idx))
...
def get_shufflenet_v2(multiplier, pretrained=False):
    net = ShuffleNetV2(width_multiplier=multiplier)
...
if __name__ == "__main__":
    ...
    for i, layer in enumerate(net.features):
        x = layer(x)
        print('{:25}\t{}\t{}'.format(layer.name, x.shape, i))
